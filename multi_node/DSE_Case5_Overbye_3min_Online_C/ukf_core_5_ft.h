/**
 * ukf_core_5_ft.h — FT-optimized C UKF for 5-Bus 2-Generator
 * Uses: BLAS-FT (cblas_dgemm), LAPACK-FT (dpotrf/dgetrf/dgetri), VML-FT (cvexp_d)
 */
#ifndef UKF_CORE_5_FT_H
#define UKF_CORE_5_FT_H
...
static inline void rk4_sigma(int n, double deltt, const double *E_abs, int ns,
    ...
    double k1_w[2 * N_SIGMA];
    double k1_d[2 * N_SIGMA];
    double PG[2 * N_SIGMA];

    double complex cexp_buf[2 * N_SIGMA];
    double complex cexp_src[2 * N_SIGMA];
    ...
    double k2_w[2 * N_SIGMA], k2_d[2 * N_SIGMA];
    double k3_w[2 * N_SIGMA], k3_d[2 * N_SIGMA];
    double k4_w[2 * N_SIGMA], k4_d[2 * N_SIGMA];
...
static inline int ukf_step(const SystemParams *sp, UKFState *st,
    ...
    for (int si = 0; si < n_sigma; si++) {
        double complex E[2], Ibus[2], Vbus[5];
        for (int i = 0; i < n; i++)
