/**
 * controller_online_5bus_opt.c — Optimized online UKF for 5-Bus (direct SHM)
 *
 * Usage: ./controller_online_5bus_opt 5bus
 */
#include "ukf_core_5.h"
...
int main(int argc, char **argv) {
    ...
    SystemParams sp;
    if (load_system_params(&sp, "system_params_5bus.bin") != 0) {
        fprintf(stderr, "ERROR: system_params_5bus.bin not found!\n");
        return 1;
    }
    ...
    while (1) {
        ...
        if (shm_read_frame(mem, cap, fsz, read_idx, z_k, NM, &k_time)) {
            ...
            printf("%.6f,%.8f,%.8f,%.8f,%.8f,%.8f\n",
                   k_time, x_out[0], x_out[1], x_out[2], x_out[3], rmse_val);
            fflush(stdout);
