/**
 * controller_online_5bus.c — Online UKF for 5-Bus 2-Generator (direct SHM)
 *
 * Usage: ./controller_online_5bus 5bus
 */
...
int main(int argc, char **argv) {
    ...
    fprintf(stderr, "[Timing] frames=%d | avg=%.1fus | min=%.1fus | max=%.1fus | total=%.3fs | fps=%.1f\n",
            count, t_avg, t_min, t_max, t_total / 1e6, count > 0 ? 1e6 / t_avg : 0.0);
