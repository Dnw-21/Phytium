...
#define NODE_BUFFER_CYCLES       2         /* 环缓冲2周期 (SRAM实时访问) */
#define NODE_BUFFER_SIZE         (SAMPLES_PER_CYCLE * NODE_BUFFER_CYCLES)   //40点
...
#define FAULT_SNAP_RAW_BYTES     (FAULT_SNAP_POINTS * sizeof(NodeSample_t)) /* 60 * sizeof(NodeSample_t) */
#define FAULT_SNAP_HEADER_BYTES  12                        /* 故障快照头 12B (记录id/start_addr/len/crc) */
#define FAULT_SNAP_TOTAL_BYTES   (FAULT_SNAP_HEADER_BYTES + FAULT_SNAP_RAW_BYTES) /* 3492B */

/* 每 LoRa 包最多发送的采样点数 */
#define LORA_HEADER_MAX   40
#define LORA_PAYLOAD_MAX  212   /* 240B - 28B帧开销 */
#define RAW_POINTS_PER_PKT       (LORA_PAYLOAD_MAX / sizeof(NodeSample_t))
#define MAX_PENDING_FAULTS       10
...
typedef struct {
    ...
    nmea_utc_time utc;          /* GPS UTC 时间 (外部传输用) */
    uint16_t ms_offset;         /* 自 UTC 时刻起的毫秒偏移 */
} NodeSample_t;
...
void node_upload_recent_20(void);              /* 直接上传缓冲区最近20点 */
