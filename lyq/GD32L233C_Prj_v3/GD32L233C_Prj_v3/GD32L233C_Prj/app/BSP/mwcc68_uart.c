...
static volatile uint16_t frame_pos[LORA_FRAME_QUEUE];   /* 帧头队列 */
static volatile uint8_t  frame_head;     /* 帧头队列头 */
static volatile uint8_t  frame_tail;     /* 帧头队列尾 */

static volatile uint32_t aux_int_cnt;

static uint8_t  usart0RxBuffer[256];
