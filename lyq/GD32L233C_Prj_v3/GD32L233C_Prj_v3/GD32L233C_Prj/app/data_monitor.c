...
static uint8_t          g_pending_fault_count;  /* 待上传故障条数 */
static uint32_t         g_fault_snap_off;       /* 故障快照Flash偏移量 */
...
void node_sample_process(const ZDataPoint_t *z, uint32_t timestamp,
                          const nmea_utc_time *gps_utc, uint16_t gps_ms)
{
    ...
    s->timestamp = timestamp;                     /* RTOS ms: 内部使用 */
    s->utc       = *gps_utc;                      /* GPS UTC: 外部传输 */
    s->ms_offset = gps_ms;
...
/*============================================================================
 *    node_upload_recent_20: 不上传时间戳, 直接上传缓冲区最近NORMAL_UPLOAD_POINTS点
 *============================================================================*/
void node_upload_recent_20(void)
{
    if (g_node.cycle_count == 0)
        return;
    ...
    d.timestamp     = xTaskGetTickCount() * portTICK_PERIOD_MS;
    d.sample_rate   = NODE_SAMPLE_RATE;
    d.battery_pct   = battery_get_percentage();
    d.health_score  = health_get_score();
    d.total_points  = NORMAL_UPLOAD_POINTS;
    send_normal_data(DATA_TYPE_NODE_HEAD, &d, sizeof(d));

    static uint8_t  raw[RAW_POINTS_PER_PKT * sizeof(NodeSample_t)];
    uint16_t sent = 0;
    /* 从最新写入位置往前取 NORMAL_UPLOAD_POINTS 点 */
    uint16_t idx  = (g_node.write_index - 1 + NODE_BUFFER_SIZE) % NODE_BUFFER_SIZE;
    while (sent < NORMAL_UPLOAD_POINTS) {
        uint16_t n = (NORMAL_UPLOAD_POINTS - sent) > RAW_POINTS_PER_PKT ? RAW_POINTS_PER_PKT : (NORMAL_UPLOAD_POINTS - sent);
    ...
    log_info("Recent upload: 20pts from idx=%u", (g_node.write_index - 1 + NODE_BUFFER_SIZE) % NODE_BUFFER_SIZE);
}
...
void upload_fault_snaps(void)
{
    ...
    for (uint8_t k = 0; k < g_pending_fault_count; k++) {
        ...
        d.node_index    = g_active_node;
        d.fault_pending = g_pending_fault_count - k - 1; /* 剩余待上传故障数 */
        d.sample_rate   = NODE_SAMPLE_RATE;
        d.battery_pct  = battery_get_percentage();
        d.health_score = health_get_score();
        ...
        while (sent < total_pts) {
            uint16_t n = (total_pts - sent) > RAW_POINTS_PER_PKT ? RAW_POINTS_PER_PKT : (total_pts - sent);
