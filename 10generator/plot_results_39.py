...
def plot_results_39(t, X_true, X_est, RMSE, n, t_SW, t_FC):
    ...
    for i in range(n):
        ...
        ax1.set_title(f'Actual Vs Estimated $\\delta_{{{i + 1}}}$ with UKF', fontsize=12)
        ...
        ax2.set_title(f'Actual Vs Estimated $\\omega_{{{i + 1}}}$ with UKF', fontsize=12)
        ax2.legend(loc='upper right')

        fig.tight_layout()
        fig.savefig(f'ukf_results_generator{i + 1}_39.png', dpi=150)
        plt.close(fig)
        print(f'  Saved ukf_results_generator{i + 1}_39.png')

    cols = min(5, n)
    rows_ang = (n + cols - 1) // cols
    rows = rows_ang * 2
    fig2, axes = plt.subplots(rows, cols, figsize=(cols * 3.5, rows * 3))
    ...
    for i in range(n):
        ...
        ax_ang.set_ylabel(f'Angle_{i + 1}', fontsize=10)
    ...
    fig2.tight_layout()
    fig2.savefig('ukf_results_all_generators_39.png', dpi=150)
    plt.close(fig2)
    print('  Saved ukf_results_all_generators_39.png')
    ...
    fig3.tight_layout()
    fig3.savefig('ukf_results_rmse_39.png', dpi=150)
    plt.close(fig3)
    print('  Saved ukf_results_rmse_39.png')
