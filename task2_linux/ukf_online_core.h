/*
 * ukf_online_core.h — 统一在线 UKF 核心 (compile-time specialization)
 * ====================================================================
 * 基于 multi_node 目录下的实时 UKF 实现, 为每个节点生成专用代码。
 *
 * 使用方法:
 ...
 *
 * 算法来源:
 *   5bus:  multi_node/DSE_Case5_Overbye_3min_Online/controller_online.py
 *   39bus: multi_node/DSE_Case39_3min_Online_Py/controller_online.py
 *   9bus:  multi_node/DSE_Calculation_UKF_9case_3minc_implementation/controller_online.c
 *
 * 与前版(ukf_core.c)的关键区别 (在线实时UKF):
 *   1. 分离 Q/R 矩阵: angle/speed/meas 各有独立 sigma
 *   2. 矩阵求逆计算 Kalman 增益 (而非 Cholesky 求解)
 *   3. 每步对称化 P = (P+P')/2
 *   4. 所有临时矩阵静态分配 (零 malloc, 避免内存碎片)
 * ====================================================================
 */
