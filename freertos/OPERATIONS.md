...
# 查看共享内存输出
ssh user@192.168.88.10 "echo 'user' | sudo -S /home/user/trace_reader"

# 保存日志到文件
ssh user@192.168.88.10 "echo 'user' | sudo -S timeout 60 /home/user/trace_reader" > /tmp/lora_log.txt
```
...
所有命令在**开发板上**执行 (`ssh user@192.168.88.10`)。
...
### 3.4 更换固件 (stop → 更换 → start，无需 reboot)

**推荐方式** — 不影响 Linux 系统，不需要 reboot：
...
sleep 1
echo 'user' | sudo -S sh -c 'echo start > /sys/class/remoteproc/remoteproc0/state'
# 验证: sudo cat /sys/class/remoteproc/remoteproc0/state → running
```
...
```bash
ssh user@192.168.88.10 "echo 'user' | sudo -S busybox devmem 0xC8000000 32"
```
...
```bash
ssh user@192.168.88.10 "dmesg | grep remoteproc"
```
...
**确认结论（2026-06-03）：所有数据都加密，无法在 FreeRTOS 内解密。**

- 终端 `lora_send_task.c` 对所有 `data_len > 0` 的数据无条件调用 `chaos_encrypt_packet()`
- Master_v3 原版把加解密都注释了（`master_recv.c L223` 解密注释、`master_poll_task.c L54` 加密注释）
- 根因：ARMCC(GD32终端) vs GCC(FreeRTOS AArch64) 的 `sinf`/`cosf` 实现不同，同样 sync_code 生成不同密钥流
- 当前方案：接收密文、透传，不做解密。详见 `DEBUG_GUIDE.md` v12-S4 章节
