                                 ...
                                 MasterDownloadBuf_t *dl, MasterNodeInfo_t *node)
{
    ...
    if (hdr.fault_type != FAULT_NONE) {
        ...
        shm_spf("Status hdr: node%d sev=%d health=%.1f pts=%d\r\n",
...
void master_recv_task(void *pvParameters)
{
    ...
    while (1) {
        ...
        shm_spf("[RECV] sync=%016llX type=0x%02X len=%d\r\n",
                (unsigned long long)pkt.sync_code, pkt.rx_type, pkt.enc_len);
...
void master_process_task(void *pvParameters)
{
    ...
    while (1) {
        ...
        shm_spf("[DEC] sync=%016llX type=0x%02X len=%d:",
                (unsigned long long)pkt.sync_code, pkt.rx_type, payload_len);
