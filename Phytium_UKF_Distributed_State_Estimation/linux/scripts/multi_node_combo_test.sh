...
sudo scripts/reload_firmware.sh /lib/firmware/openamp_core0.elf > /tmp/reload_${TAG}.log 2>&1 || {
    echo "[ERROR] 固件热重载失败，详情见 /tmp/reload_${TAG}.log" | tee -a "$LOG"
    exit 1
}
sleep 2

sudo bin/reset_shm_arm64

# 预热 RPMsg
nohup sudo bin/start_sim_nodes 1 > /tmp/sim_prime_combo.log 2>&1 &
PRIME_PID=$!
sleep 6
sudo kill -9 $PRIME_PID || true
sleep 1

# 基础 5bus/9bus/39bus (正常模式, 更新 SHM 读指针)
nohup sudo taskset -c 0 env UKF_DELTT=0.002 bin/ukf_pipeline_5bus  > /tmp/combo_5bus_${TAG}_base.csv  2> /tmp/combo_5bus_${TAG}_base.log  &
nohup sudo taskset -c 0 env UKF_DELTT=0.002 bin/ukf_pipeline_9bus  > /tmp/combo_9bus_${TAG}_base.csv  2> /tmp/combo_9bus_${TAG}_base.log  &
nohup sudo taskset -c 2 env UKF_DELTT=0.004 bin/ukf_pipeline_39bus > /tmp/combo_39bus_${TAG}_base.csv 2> /tmp/combo_39bus_${TAG}_base.log &

# 额外 5bus 只读实例
for i in $(seq 1 "$E5"); do
    nohup sudo taskset -c 0 env UKF_DELTT=0.002 UKF_READONLY=1 bin/ukf_pipeline_5bus > /tmp/combo_5bus_${TAG}_${i}.csv 2> /tmp/combo_5bus_${TAG}_${i}.log &
done
# 额外 9bus 只读实例
for i in $(seq 1 "$E9"); do
    nohup sudo taskset -c 0 env UKF_DELTT=0.002 UKF_READONLY=1 bin/ukf_pipeline_9bus > /tmp/combo_9bus_${TAG}_${i}.csv 2> /tmp/combo_9bus_${TAG}_${i}.log &
done
# 额外 39bus 只读实例
for i in $(seq 1 "$E39"); do
    nohup sudo taskset -c 2 env UKF_DELTT=0.004 UKF_READONLY=1 bin/ukf_pipeline_39bus > /tmp/combo_39bus_${TAG}_${i}.csv 2> /tmp/combo_39bus_${TAG}_${i}.log &
done

# 启动 FreeRTOS 数据生成
nohup sudo bin/start_sim_nodes 1 > /tmp/sim_combo_${TAG}.log 2>&1 &
