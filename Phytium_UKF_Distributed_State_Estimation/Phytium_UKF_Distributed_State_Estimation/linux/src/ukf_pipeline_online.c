/*
 * ukf_pipeline_online.c — 在线 UKF 状态估计 Pipeline
 * ==========================================================
 * 从 FreeRTOS 共享内存读取电力系统仿真数据，每帧执行一次 UKF
 * 状态估计，结果以 CSV 形式输出到 stdout，状态信息输出到 stderr。
 *
 * 数据流:
 ...
 *
 * 编译:
 *   aarch64-linux-gnu-gcc -O2 -static -DUKF_NODE_5BUS \
 *       -o ukf_pipeline_5bus  ukf_pipeline_online.c -lm
 *   aarch64-linux-gnu-gcc -O2 -static -DUKF_NODE_9BUS \
 *       -o ukf_pipeline_9bus  ukf_pipeline_online.c -lm
 *   aarch64-linux-gnu-gcc -O2 -static -DUKF_NODE_39BUS \
 *       -o ukf_pipeline_39bus ukf_pipeline_online.c -lm
 *
 * 输出:
 *   stdout: time,delta1,...,deltaN,omega1,...,omegaN,RMSE
 *   stderr: [ukf-NODE] t=4.0s frames=251 X=[0.31,0.68] rmse=0.0001 lat=55us
 *
 * 内存占用: 纯静态分配，零 malloc/free
 *   5bus:  ~20KB   9bus:  ~30KB   39bus: ~120KB
 * ==========================================================
 */
