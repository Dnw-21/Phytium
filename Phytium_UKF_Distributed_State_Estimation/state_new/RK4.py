...
def rk4(n, deltt, E_abs, ns, X_sigma, PM, M, D, Ybusm):
    ...
    E2 = np.tile(E_abs.reshape(-1, 1), (1, 2 * ns)) * np.exp(1j * (X_sigma[:n, :] + k1_delta / 2))
    I2 = Ybusm @ E2
    PG2 = np.real(E2 * np.conj(I2))
    k2_w = deltt * (M_rep**(-1) * (PM_rep - PG2 - (D_rep * (X_sigma[n:ns, :] + k1_w / 2))))
    k2_delta = deltt * (X_sigma[n:ns, :] + k1_w / 2)

    E3 = np.tile(E_abs.reshape(-1, 1), (1, 2 * ns)) * np.exp(1j * (X_sigma[:n, :] + k2_delta / 2))
    I3 = Ybusm @ E3
    PG3 = np.real(E3 * np.conj(I3))
    k3_w = deltt * (M_rep**(-1) * (PM_rep - PG3 - (D_rep * (X_sigma[n:ns, :] + k2_w / 2))))
