/**
 * controller_online_9bus_opt.c — Optimized online UKF for 9-Bus (direct SHM)
 *
 * Usage: ./controller_online_9bus_opt 9bus
 */
...
/* ================================================================
 * ukf_init — Initialize persistent UKF state
 * ================================================================ */
...
/* ================================================================
 * Main — direct SHM streaming mode with timing
 * ================================================================ */
int main(int argc, char **argv) {
    if (argc < 2) {
        fprintf(stderr, "Usage: %s <9bus>\n", argv[0]);
        return 1;
    }
    ...
    SystemParams sp;
    if (load_system_params(&sp, "system_params_9bus.bin") != 0) {
        fprintf(stderr, "ERROR: system_params_9bus.bin not found!\n");
        return 1;
    }
    fprintf(stderr, "[Init] Loaded system_params_9bus.bin (n=%d, s=%d, nm=%d)\n",
            sp.n, sp.s, sp.nm);
    ...
    /* ---- Calculate initial read offset (catch up to latest) ---- */
    ...
    while (1) {
        ...
        if (shm_read_frame(mem, cap, fsz, read_idx, z_k, NM, &k_time)) {
            ...
            printf("%.6f,%.8f,%.8f,%.8f,%.8f,%.8f,%.8f,%.8f\n",
                   k_time, x_out[0], x_out[1], x_out[2],
