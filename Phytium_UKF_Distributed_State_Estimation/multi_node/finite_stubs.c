...
/* Local fmod/fmodf to avoid GLIBC_2.38 version requirement on dev board */
...
static float local_fmodf(float x, float y) {
    if (y == 0.0f) return 0.0f;
    float q = x / y;
    float truncated = (float)((long long)q);
    return x - truncated * y;
}
...
MAKE_STUB_F(exp)   MAKE_STUB_F(exp2) /* exp10 not needed by VML-FT */ MAKE_STUB_F(expm1)
MAKE_STUB_F(log)   MAKE_STUB_F(log2) MAKE_STUB_F(log10) MAKE_STUB_F(log1p)
MAKE_STUB_F(sqrt)  MAKE_STUB_F(cbrt)
MAKE_STUB_F2(atan2) /* fmod local */ MAKE_STUB_F2(pow) MAKE_STUB_F2(remainder)
...
MAKE_STUB_D(exp)   MAKE_STUB_D(exp2) /* exp10 not needed by VML-FT */ MAKE_STUB_D(expm1)
MAKE_STUB_D(log)   MAKE_STUB_D(log2) MAKE_STUB_D(log10) MAKE_STUB_D(log1p)
MAKE_STUB_D(sqrt)  MAKE_STUB_D(cbrt)
MAKE_STUB_D2(atan2) /* fmod local */ MAKE_STUB_D2(pow) MAKE_STUB_D2(remainder)
MAKE_STUB_D(lgamma) MAKE_STUB_D(gamma)

/* fmod stubs using local implementations to avoid GLIBC_2.38 dependency */
