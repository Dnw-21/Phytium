...
uint16_t usart1_data_available(void);   //查询USART1是否有数据可读
uint16_t usart1_read_data(uint8_t* buf, uint16_t maxLen);   //读取USART1数据
void     usart1_clear_buffer(void);   //清空USART1接收缓冲区
uint16_t usart1_peek_data(uint8_t* buf, uint16_t maxLen);  

uint8_t  usart1_frame_available(void);   //查询USART1是否有完整帧可读
