...
> **目标**：将 ATK-MWCC68D LoRa 模块接到飞腾派 PE2204，
> 实现 FreeRTOS 主控侧通过 UART2 直接收发 GD32 终端节点的真实 LoRa 数据。
>
> **当前状态**：LoRa 主控链路以真实硬件为目标，串口以 UART2 为准；FreeRTOS 实际运行在 CPU1，但设备树/remoteproc 仍写 CPU3。
...
│                                                                  │
│  ┌──────────────────┐         ┌───────────────────────────────┐ │
│  │ Linux (接收/显示)│ RPMsg   │ FreeRTOS 主控侧                │ │
│  │                   │◄───────►│                                │ │
...
| 决策                   | 说明                                                                                  |
| -------------------- | ----------------------------------------------------------------------------------- |
| **FreeRTOS 直连 LoRa** | LoRa 模块的 UART 和 GPIO 由 FreeRTOS 主控侧直接控制，**不经 Linux**。Linux 只能通过 RPMsg 获取已处理的数据。 |
| **Linux 不占用 LoRa 串口**  | LoRa 串口以 UART2 为准，Linux 不应占用该外设，避免与 FreeRTOS 寄存器访问冲突。                           |
| **真实链路与面板分离**          | LoRa 主控链路是真实硬件路线；UKF Dashboard 当前仍使用 `state_new/` 模拟数据，后续再打通。                     |
| **CPU 口径**     | FreeRTOS 实际运行在 CPU1，但设备树/remoteproc 仍写 CPU3。       |
...
LoRa 主控路线当前以真实硬件链路为准；UKF Dashboard 当前仍使用模拟数据，尚未与 LoRa→FreeRTOS→Linux 数据打通。
...
|   Pin 8   | UART2\_TXD | UART2 TX    | RXD            |  黄色  |
|   Pin 10  | UART2\_RXD | UART2 RX    | TXD            |  橙色  |
...
PE2204 当前接线口径以 UART2 为准；历史 UART3 记录仅作为排错背景，不再作为当前接线事实。
...
AT+AIRRATE=6           # 设置空中速率 62.5kbps (索引6)
...
| 文件                           | 修改内容                                    | 状态 |
| ---------------------------- | --------------------------------------- | :-: |
| `freertos/src/master_recv.c` | 接入真实 LoRa 接收入口，保留历史仿真作回归测试 | ✅ 已完成 |
| `freertos/src/master_cmd.c`  | 后续按主控移植路线接入 UART2/分时接收控制逻辑 | 进行中 |
| `freertos/src/lora_uart.c`   | UART2 LoRa 串口驱动 (RX环形缓冲 + 帧提取) | 当前口径 |
| `freertos/src/rpmsg-echo_os.c` | 新增 DEVICE_LORA_CTRL(0x0022) RX开关控制 | ✅ 已完成 |

### 5.2 当前 LoRa 接收口径

当前 LoRa 主控链路不再以仿真开关作为事实来源，而是以真实硬件 UART2 接入为准。历史 `USE_LORA_SIMULATION` 片段只用于理解早期切换方式或无硬件回归测试。

### 5.3 UART2 驱动实现参考

当前实际接线为 UART2（J1 Pin8/Pin10）。以下 PL011 寄存器说明保留为移植参考，具体基址/IRQ 以当前 `lora_uart.c` 和 SDK 参数为准。
...
#### 5.3.2 UART2 外设基址（PE2204 专用）

```c
// ★ 当前 LoRa 接线口径，以 SDK fparameters_comm.h 为准
#define UART2_BASE     0x2800e000U
#define UART2_CLK_HZ   100000000U
...
{
    // 1. 禁用 UART2
    UART_REG(UART2_BASE, UART_CR) = 0;

    // 2. 配置波特率: 115200 @ 100MHz
    UART_REG(UART2_BASE, UART_IBRD) = 54;
    UART_REG(UART2_BASE, UART_FBRD) = 16;

    // 3. 配置行控制: 8N1, 使能 FIFO
    UART_REG(UART2_BASE, UART_LCR_H) = UART_LCRH_WLEN_8 | UART_LCRH_FEN;

    // 4. 配置 FIFO 中断级别: RX=1/8满时触发中断
    UART_REG(UART2_BASE, UART_IFLS) = 0x00;

    // 5. 使能 RX 中断
    UART_REG(UART2_BASE, UART_IMSC) = UART_IMSC_RXIM;

    // 6. 使能 UART2
    UART_REG(UART2_BASE, UART_CR) = UART_CR_UARTEN | UART_CR_RXE | UART_CR_TXE;

    // 7. 配置 GPIO2_10 为输入 (AUX 状态检测)
    UART_REG(GPIO2_BASE, GPIO_DIR_OFFSET) &= ~(1U << 10);

    // 8. 注册中断处理函数 ← 需要根据 SDK 中断框架注册
    // FInterruptInstall(FUART2_IRQ_NUM, lora_uart_isr, NULL);
...
static void lora_uart_isr(void *arg)
{
    ...
    u32 mis = UART_REG(UART2_BASE, UART_MIS);

    if (mis & UART_IMSC_RXIM) {
        // 读空 RX FIFO
        while (!(UART_REG(UART2_BASE, UART_FR) & UART_FR_RXFE)) {
            u8 byte = (u8)(UART_REG(UART2_BASE, UART_DR) & 0xFF);
            ring_put(byte);
        }
    }

    // 清除中断
    UART_REG(UART2_BASE, UART_ICR) = mis;
}
```
...
> 当前路线以真实 LoRa/UART2 数据为准；如需无硬件回归，可用历史模拟入口验证 `parse_frame()`，但不要把它作为当前主链路事实。
>
...
编辑 `freertos/src/master_cmd.c`，在 `send_lora_cmd()` 中按当前 UART2 口径添加发送逻辑：
                           ...
                           const uint8_t *params, uint8_t param_len)
{
    ...
    // ★ 原代码通过 RPMsg 发送，当前按 UART2 真实 LoRa 链路发送
    // 方式1: 直接用 UART FIFO 发送
    for (int i = 0; i < (5 + enc_len); i++) {
        while (UART_REG(UART2_BASE, UART_FR) & UART_FR_TXFF)
            ;  // 等待 TX FIFO 有空位
        UART_REG(UART2_BASE, UART_DR) = g_lora_pkt[i];
    }
...
```bash
# 1. 确认当前 LoRa UART2 不被 Linux 侧占用；历史 UART3 检查仅作为早期排错背景
ls /dev/ttyAMA*

# 2. 加载 RPMsg 模块
modprobe rpmsg_char rpmsg_ctrl

# 3. 启动 FreeRTOS 主控侧（实际 CPU1，设备树写 CPU3）
...
|  2  | UART2 寄存器可访问   | FreeRTOS 启动后写读 CR 寄存器 | 读回值与写入值一致         |
|  3  | GPIO2\_10 状态正确 | 读 GPIO 数据寄存器          | AUX 高电平(模块就绪)     |
|  4  | UART2 能收到字节    | 环形缓冲区有数据              | ring\_avail() > 0 |
...
在 FreeRTOS 的 `master_recv_task` 中添加调试日志，验证 UART2 接收：
...
// 在 master_recv_task 循环中添加:
{
    ...
    if ((now - last_print) > pdMS_TO_TICKS(5000)) {
        OPENAMP_DEVICE_INFO("UART2 ring avail: %lu bytes", ring_avail());
        last_print = now;
    }
}
```
...
dmesg -w | grep -i "lora\|uart2\|master"
...
> 当前 LoRa 接线口径是 UART2；本节保留早期 UART3 调试踩坑，只作为历史排错背景。
...
### 9.2 历史 UART3 设备树占用记录

> 当前 LoRa 接线口径是 UART2；本节说明早期 UART3 overlay 为什么不再作为当前路线。
...
| 项目     | 值                                                      |
| ------ | ------------------------------------------------------ |
| **现象** | UART2 有数据但 ISR 不触发                                     |
| **原因** | PE2204 使用 GICv3 中断控制器，当前 UART2 IRQ 为 117，需要正确注册并路由到 FreeRTOS 实际运行 CPU |
| **解决** | 使用 SDK 的中断接口注册，参考当前 UART2 中断示例   |
...
| 参数             | 值               |
| -------------- | --------------- |
| LoRa UART 基址  | UART2 / 0x2800E000 |
| LoRa UART IRQ   | 117 (GICv3 SPI) |
| UART 时钟        | 100MHz          |
| GPIO2 基址       | 0x28036000      |
| GPIO3 基址       | 0x28037000      |
| UART2 TXD 引脚   | J1 Pin8         |
| UART2 RXD 引脚   | J1 Pin10        |
| GPIO2\_10 (AUX)  | J1 Pin7         |
| GPIO3\_1  (MD0)  | J1 Pin11        |
| GICv3 基址       | 0x30800000      |
| FreeRTOS CPU 口径 | 实际 CPU1，设备树/remoteproc 写 CPU3 |
