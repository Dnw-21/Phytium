...
```bash
# 确认开发板在线
ping -c 1 192.168.88.11

# 确认 SSH 可用
sshpass -p 'user' ssh -o StrictHostKeyChecking=no user@192.168.88.11 "echo ok"
```
...
- **故障时段标记**: 当前模拟数据故障在 5s 和 15s 出现，故障窗口由后端参数动态下发
- **节点传输状态**: 实时显示各节点数据传输量和连接状态
- **实时播放**: 使用 `state_new/` 生成/缓存的 180s 模拟数据，支持开始/暂停/重置控制
...
3. **观察曲线**: 
   - 实线 = 估计值 (UKF 输出)
   - 虚线 = 真实值 (参考数据)
4. **故障检测**: 当前模拟数据会在 5s 和 15s 附近触发故障区域高亮
...
├── dashboard_server.py          # Flask 服务端 + UKF 引擎，当前唯一 Dashboard，端口 5000
├── templates/dashboard.html     # Web 前端 (Chart.js)
└── ...                          # Dashboard 缓存、天气风险和通知相关文件
...
- Dashboard 当前使用 `state_new/` 的模拟数据进行演示
- 实际部署时需要接入 LoRa→FreeRTOS→Linux 的真实开发板数据
- 当前模拟故障在 5s 和 15s 出现，故障窗口由后端参数动态下发
- 循环播放时图表会自动清空并重新开始