...
| 2026-06-17 | 面板标题移除“高亮”字样；横坐标改为各节点自身北京时间采样时刻，刻度格式统一为“分:秒.毫秒”；切换显示节点时横坐标随该节点原始时间戳刷新 |
| 2026-06-18 | 修复 `dashboard_data.json` 时间单位 bug：`time_sec` 与 `fault_snapshots.center_sec/times` 统一为秒（原误用 step 序号）；修正 `prep/build_data_1000hz.py` 移动故障波形时覆盖时间戳的问题；重新生成数据并部署到飞腾派 |
| 2026-06-18 | 修复三节点轮播卡顿/残留数据：服务端只给当前活跃节点追加历史；前端切换节点时彻底清空曲线、关闭动画、避免双定时器重复刷新；VNC 浏览更流畅 |
| 2026-06-18 | 修复故障回放提前泄露：回放列表只在当前仿真时间到达该故障中心后才显示，避免第一个故障触发时后续节点回放已出现；时间范围显示改为毫秒精度 |
| 2026-06-18 | 修复“重置”后曲线未清空：前端 destroy 并重建 Chart.js 实例、丢弃重置前已发出的旧 API 响应；服务端 reset 后停止仿真线程并清空历史 |
| 2026-06-18 | 修复三节点故障触发时机与轮播不同步：将 node_1/2/3 的故障窗口分别调整到 step 9-11/12-14/15-17，使其与各自活跃窗口完全对齐；面板、推送、故障回放三者一致 |
| 2026-06-18 | 修复故障高亮失效：setChartsFaultHighlight 传入的 node ID 错误（传了 '1/2/3' 而不是 'node_1/2/3'），导致曲线变红逻辑从未命中；改为正确 ID 后 node_2/3 故障时面板曲线也会变红 |
| 2026-06-18 | 修复三节点状态返回不全：_get_enabled_nodes 在 3 节点模式下被 config.json 的 enabled 开关限制，且 _make_node_frame 因物理心跳缺失把 node_2/3 标记为 HIDDEN，导致面板收不到后两个节点故障；已改为 3 节点演示模式始终返回全部节点并忽略心跳 |
