...
uint8_t LoRa_SetWlrate(uint8_t chn, uint8_t wlrate)
{
    usart1_clear_buffer();
    char cmd[20];
    sprintf(cmd, "AT+WLRATE=%d,%d", chn, wlrate);
    printf("[INFO] Send: %s\r\n", cmd);
    for(int i=0; cmd[i]; i++) {
        while(usart_flag_get(USART1, USART_FLAG_TBE) == RESET);
        usart_data_transmit(USART1, cmd[i]);
...
void LoRa_Init(void)
{
    ...
    cpu_delay_ms(100);
    LoRa_SetWlrate(LORA_CHN, LORA_RATE);
    cpu_delay_ms(100);
...
void ProcessBackToNormal(void)
{
    ...
    cpu_delay_ms(200);
    LoRa_SetWlrate(LORA_CHN, originalWlrate);
    cpu_delay_ms(200);
