...
void send_lora_cmd(uint8_t node_id, uint8_t cmd_code, const uint8_t *params, uint8_t param_len)
{
    ...
    send_node_data_with_ack(g_lora_pkt, send_len, cmd_code, &dest_addr, 3, now);
...
void master_poll_task(void *pvParameters)
{
    ...
    while (1) {
        ...
        if (fault_count > 0) {
            TickType_t tier2_deadline = xTaskGetTickCount() + pdMS_TO_TICKS(MASTER_POLL_TIER2_BUDGET_MS);
