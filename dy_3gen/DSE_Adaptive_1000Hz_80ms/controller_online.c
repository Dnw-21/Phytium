...
int main(int argc, char **argv) {
    ...
    /* ---- Print CSV header with fault info ---- */
    printf("# time,delta1,delta2,delta3,omega1,omega2,omega3,RMSE  fault_tSW=%.4f  fault_tFC=%.4f\n",
           sp.t_SW, sp.t_FC);
    fflush(stdout);
    ...
    while (fgets(line, sizeof(line), input)) {
        ...
        /* Supports both "Vreal1..Vimag9" and "Vmag1..Vangle9" formats */
        char *tok = strtok(line, ",");
        double z_k[NM];
        int has_ts = 0;
        double k_time = count / (double)sp.fs;  /* default: from step count */
        ...
        printf("%.6f,%.8f,%.8f,%.8f,%.8f,%.8f,%.8f,%.8f\n",
               ...
               rmse_val);
        fflush(stdout);  /* flush each line for real-time consumers */
